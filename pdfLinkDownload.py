...
def download_oferts_pdf(links, output_folder):
    downloaded_files = []
    token = conf.token
    tenat = conf.tenat
    ...
    for link in links:
        ...
        if response.status_code == 200:
            content_disposition = response.headers.get('Content-Disposition')
            filename = f"Oferta{ofertNumber}.pdf"
            file_path = os.path.join(output_folder, filename)
            with open(file_path, 'wb') as f:
                f.write(response.content)
            log_mesajes(f"Descarga de Archivo: {filename}", conf.ruta_global, "log.txt", True)
...
#oferts = [6,75]
ofertsLinks = [conf.getOfertsUrl + str(ofert) for ofert in oferts]
download_oferts_pdf (ofertsLinks,folder_path) # para que esto no de error hay que asegurar obtener el token de un usario logeado
